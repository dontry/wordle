...
const Tile: React.FC<TileProps> = ({ status, children }) => {
  const classes = classNames(
    ...
    {
      "bg-green-400 text-white": status === "correct",
      "bg-yellow-200 text-white": status === "include",
      "bg-gray-400 text-white": status === "incorrect",
      "border-slate-400": children !== '' && status === "empty",
    },
  );

  return <div className={classes} role="gridcell" data-testid={children}> {children}</div>;
};
